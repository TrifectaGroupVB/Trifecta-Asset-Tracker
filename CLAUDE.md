...
Mobile-first web app for managing commercial kitchen equipment and service requests across **multiple restaurant locations** under one operator. QR tags on equipment link to equipment pages and service-request forms; a single PIN-protected dashboard manages requests, techs, parts, and tags **combined across every location**.

**Locations** (the `Location` model, see DATA MODEL): Waterman's Surfside Grille ("Grille" with the E, per their logo), 415 Atlantic Ave; Chix on the Beach, 701 Atlantic Ave; The Shack on 8th, 715 Atlantic Ave — all Virginia Beach, VA 23451. Never hardcode a restaurant's name/address/logo in a component — always pull from the `Location` row via `Equipment.restaurant` or `TagBatch.restaurant`. Logos live at `public/brand/{slug}-logo-full.png` (transparent background, works on the dark theme). Used at ~2in wide on sticker sheets (legible there; below ~100px the text blurs, avoid going smaller). `public/brand/watermans-mark.png` is a standalone fish-tail glyph cropped from the full logo — not currently used anywhere, kept in case a future small-format spot (favicon, etc.) needs it. **Chix's asset is a wordmark only** (`Chix_Logo_White.avif` from Parker → converted to PNG), not the circular badge used in their other marketing — it was already a transparent white cutout, so no background removal was needed or risked against the lettering. `Location.printLogoUrl` (nullable, falls back to `logoUrl` when unset) exists for exactly this case: Chix's white wordmark is invisible on the sticker sheet's white background, so print materials (`/dashboard/tags/print/[batchNumber]`, the export route, `stickerExport.ts`) use `printLogoUrl` when set — for Chix that's `chix-logo-print.png`, a navy recolor made by extracting the alpha channel and compositing it over a solid navy fill (preserves the exact glyph shape, no re-tracing). **Shack's asset is the full circular badge**, background-removed locally from a white-background source JPG via ImageMagick flood-fill (`-fuzz 12% -fill none -draw "color X,Y floodfill"` from all four corners) — picked over sending the logo to a cloud background-removal API since a local tool was on hand and did the job cleanly (checked at pixel level for white fringing before finalizing). **Sticker logo sizing is a bounding box, not just a width:** `LOGO_BOX_WIDTH_IN` × `LOGO_BOX_HEIGHT_IN` (2.1in × 1.28in, both in `stickerExport.ts` and hand-synced to the `h-[1.28in] w-[2.1in]` wrapper in the print page) — every logo is fit *inside* that box (aspect preserved, shrink-only, `fit: "inside"` in sharp / `object-contain` on-screen) and centered within it. A fixed-width-only approach broke on Shack's square badge: at a fixed 2.1in width it rendered 2.1in tall (vs. ~1.28in for Waterman's landscape logo), overflowing the 3.4in sticker and pushing the restaurant-name caption off the bottom. Note: `Equipment.location` (string field) means the spot *within* a restaurant ("Cook line — station 2") — unrelated to the `Location` model despite the name collision; the relation field on `Equipment`/`TagBatch` is deliberately called `restaurant` to keep the two apart in code.
...
- `/` — home: dynamic logo + links to Equipment / Report a problem / Dashboard. Corner toggle (top-right) switches the visitor's location — selection is stored in the `tat_location` cookie (`src/lib/location.ts`) and drives every public page below. Combined view isn't offered here on purpose — customers scan a QR or pick a location, they don't need a "see everything" mode.
- `/equipment` — public index scoped to the visitor's selected location (`resolveLocation()`), `?q=` searches name/model within it. Shows a "Switch location" link back to `/` when more than one location exists.
- `/equipment/[id]` — public detail (QR scan target): DataPlate header, sticky OVERVIEW/MANUALS/PARTS/HISTORY tabs, part-order flow (`EquipmentTabs` client component + `submitPartOrder` server action → emails admin, includes the equipment's restaurant name in the email body), Quick Add History (staff-only inline form on the HISTORY tab that writes a `ServiceRecord` directly, no `ServiceRequest` needed). Shows an "Edit" badge on the DataPlate linking straight to `/dashboard/admin/equipment/[id]` when `hasDashboardSession()` is true (PIN or biometrics — same session cookie either way); absent entirely when unauthenticated, not a link that bounces to a PIN prompt.
- `/request` — public service-request form (one screen), scoped to a location resolved in this priority order: `?equipment={id}` pins it to that equipment's own restaurant (overrides everything — a direct equipment link should never show the wrong location's picker) → `?location={slug}` (set by a scanned station tag) → the `tat_location` cookie. Photo uploads land in `/public/uploads/requests/`. No email on create (per spec — emails are for assignment + part orders). Location is never asked for explicitly on this form — it's always implicit through which equipment gets picked, and `ServiceRequest` has no location field of its own; it's derived via `equipment.restaurant` wherever displayed (dashboard queue, request detail).
- `/dashboard` — PIN-gated queue combined across every location (stats strip, status chips, **location chips**, search, urgent-first sort; URGENT+OPEN+>4h badges pulse). Each row shows a location badge when more than one location exists. Any dashboard page renders the PIN pad in place when there's no session — one PIN unlocks the whole combined dashboard, there's no per-location PIN.
- `/dashboard/requests/[id]` — request detail: assign (emails tech, `[URGENT]` subject tag), Assigned → In Progress → Completed (completion writes a ServiceRecord), timeline + append-only notes (`RequestEvent`). Equipment summary plate includes a "Restaurant" field when more than one location exists.
- `/dashboard/admin` — equipment list (location badge per row) + techs CRUD (shared across all locations — one roster, one admin email) + Locations section (name/address editable per location; logoUrl shown read-only, swap the file in `public/brand/` to change it) + settings (admin email, PIN change) + biometric device enrollment (`BiometricEnrollButton`, list + remove). `/dashboard/admin/equipment/[id]` (or `new`) — full equipment editor (Restaurant select first when >1 location exists, then details, photo, specs, manuals-by-file-or-URL, parts w/ vendor links; delete blocked when service history exists)
- `/t/[code]` — **the only URL a QR sticker ever encodes.** Routes by tag state: voided/unknown → "isn't active" page; EQUIPMENT → equipment page; SERVICE_REQUEST → `/request?location={tag's restaurant slug}&from={label}` (the location rides along so the request form shows the right restaurant's equipment regardless of the visitor's cookie); UNASSIGNED → PIN-gated setup wizard (`TagWizard`: equipment new/existing, or request station), scoped to the tag's own restaurant — a blank tag generated for Chix can only be pointed at Chix equipment (enforced both by filtering the list shown and by a server-side check in `assignTagToEquipment`), and "new unit" creation inherits the tag's batch's `restaurantId` automatically. Wizard actions must NOT call revalidatePath — it re-runs the route mid-action and the fresh redirect eats the confirmation screen
- `/dashboard/tags` — batch generator (location select when >1 location exists, default 20, max 100 stickers), tag table (role badges, **location badge**, points-to, void/restore, reassign→wizard), blanks filter. `/dashboard/tags/print/[batchNumber]` — printable sticker sheet: 2.5in × 3.4in, dashed cut lines, the batch's own restaurant logo above the QR (not overlaid — QR is untouched, no error-correction tradeoff), QR → `{BASE_URL}/t/{code}`, mono code fallback, caption is the batch's restaurant name + "SCAN ME" (never hardcode the restaurant), pure black on white (print: chrome is `print:hidden`). **Physical dimensions are hand-synced between this page's Tailwind arbitrary-value classes and the constants in `src/lib/stickerExport.ts`** — Tailwind can't consume a shared JS constant, so if one changes, update the other.
- `/dashboard/tags/print/[batchNumber]/export` — Route Handler (not a page) that renders every sticker in the batch as an individual 300 DPI PNG (`src/lib/stickerExport.ts`, via `sharp`, using the batch's own restaurant name + logoUrl) and returns them zipped (`jszip`) for handing off to an outside vinyl/sticker print vendor. One file per sticker since each has a different QR. Text (code/name/scan-me) uses plain system fonts, not the app's Barlow Condensed/JetBrains Mono — acceptable since the logo image carries the actual brand fidelity. PIN-gated same as other dashboard routes, but returns a redirect (no page to render) rather than `PinGate` when unauthenticated. Verified by decoding every PNG in a real downloaded zip — see git history for the test approach if this ever needs re-verifying.
...
- **Location**: one restaurant (slug, name, address, logoUrl, printLogoUrl nullable — see STACK for why print sometimes needs a different asset than the app UI). `Equipment` and `TagBatch` each belong to exactly one, via a `restaurantId` FK / `restaurant` relation (named `restaurant`, not `location`, to avoid clashing with `Equipment.location`). Nothing else carries a direct location reference — `ServiceRequest`, `ServiceRecord`, `Part`, etc. all derive their location by joining through `equipment.restaurant` when it needs to be shown.
- **Equipment**: restaurantId (→ Location), name, manufacturer, model, serial, location (in-restaurant spot, e.g. "Cook line — station 2" — NOT the Location model), installDate, photoUrl, warrantyExpires, notes
...
- **Tech**: name, email, phone — shared across every location, not scoped to one
...
- **Tag**: code (8-char unique URL-safe string), role (`UNASSIGNED|EQUIPMENT|SERVICE_REQUEST`), equipmentId (nullable), label (e.g. "Front kitchen wall"), batch, assignedAt, voided. A tag's location is always resolved via `tag.batch.restaurant` — a tag has no restaurantId of its own.
- **TagBatch**: batchNumber, restaurantId (→ Location), count — tags are printed in batches for one location at a time, assigned later
- **Setting**: key/value store (`adminEmail`, `dashboardPin`) — shared across all locations, since there's one combined dashboard and one PIN. (`restaurantName`/`restaurantAddress` used to live here in the single-restaurant era; superseded by the `Location` table.)
...
- [x] Multi-location: `Location` model + `restaurantId` on Equipment/TagBatch (migration backfilled all existing data to Waterman's), home-screen location toggle (`tat_location` cookie via `src/lib/location.ts`), public Equipment/Request pages scoped per location, combined dashboard queue with location chips/badges, per-location tag batches + branded sticker sheets, admin Locations section. All three locations' real logos are in place (`public/brand/chix-logo-full.png`, `shack-logo-full.png`, `watermans-logo-full.png`) — verified rendering correctly on the home screen and printed sticker sheets for all three.
...
**⚠️ LOCAL AND PRODUCTION USE DIFFERENT DATABASES. Verify before trusting either.**

This was wrong here for a while and it caused a live outage on 2026-07-30 — the note used to say local `.env` pointed at the *same* database as production. It does not. The Vercel project was re-created on 2026-07-25 (`prj_Rrd7zKkLQIEYo3mtE20WeaBmHp3a`, production URL `trifecta-asset-tracker-eta.vercel.app`) with a **fresh Neon database attached via the Neon integration**, while local `.env` still points at the **older** database from the original deploy. They hold different data: a unit created in production will not appear locally, and vice versa.

**What went wrong, so it doesn't repeat:** a migration adding `Part.refNumber`/`Part.qty` was run locally with `prisma migrate dev`. That migrated the *old* database. Production's database never got it, so when the code shipped, every `/equipment/[id]` page — the QR-sticker landing page — returned 500 with `P2022: The column Part.refNumber does not exist`. Fixed by rolling production back to `main` and making the build migrate its own database (below).

**Rules now:**
- **Never assume which database you're on.** Before any schema work, check: `npx vercel env ls` shows which variables production carries, and the production logs will name any column mismatch instantly. Secret *values* are redacted from `vercel env pull` for this account, so you cannot read production's `DATABASE_URL` — which is exactly why migrations run in the build instead.
- **Migrations are applied by the deploy, not by hand.** `npm run build` is `prisma migrate deploy && next build`, so every deployment migrates the database it is actually configured to use, with its own credentials. If the migration fails the build fails, and the previous deployment keeps serving — production can't end up ahead of its schema. Use `npm run build:no-migrate` for a local production build against a database you don't want touched.
- `prisma migrate dev` locally still only ever touches the old local database. That's fine for iterating; it is **not** how production gets migrated.

Running `npm run dev` locally reads/writes the OLD database's real data (still real data — just not the data production serves). `BLOB_READ_WRITE_TOKEN` is commented out locally on purpose, so local dev photo/PDF uploads fall back to `/public/uploads` instead of landing in the production Blob store — uncomment only for one-off tasks (like re-seeding) that intentionally need to write to prod Blob, then comment it back out. If ongoing local feature testing resumes, consider a separate Neon branch database for dev.
